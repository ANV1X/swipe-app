...
export interface SharedItem {
  product_id: string; title: string; brand: string | null
  price: number; price_old: number | null; image_url: string
  marketplace: string; external_url: string
  added_by: string; added_by_name: string; added_at: string
}
...
// ─── Products ──────────────────────────────────────────────────────────
export async function fetchProducts(p?: { category?: string; price_max?: number; gender?: string }): Promise<Product[]> {
...
export async function removeFromWishlist(product_id: string) {
  const res = await fetch(`${BASE_URL}/wishlist/${product_id}`, { method: 'DELETE', headers: headers() })
...
export async function joinSharedWishlist(id: string): Promise<SharedWishlistData> {
  const res = await fetch(`${BASE_URL}/shared-wishlist/${id}/join`, { method: 'POST', headers: headers() })
  if (!res.ok) throw new Error('join failed')
  return res.json()
}
export async function addToSharedWishlist(id: string, product_id: string): Promise<SharedWishlistData> {
  const res = await fetch(`${BASE_URL}/shared-wishlist/${id}/items/${product_id}`, { method: 'POST', headers: headers() })
  if (!res.ok) throw new Error('add failed')
  return res.json()
}
export async function removeFromSharedWishlist(id: string, product_id: string) {
  const res = await fetch(`${BASE_URL}/shared-wishlist/${id}/items/${product_id}`, { method: 'DELETE', headers: headers() })
  if (!res.ok) throw new Error('remove failed')
  return res.json()
}
